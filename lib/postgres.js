...
class Postgres {
  ...
  async ensureSchema() {
    await this.pool.query(`CREATE TABLE IF NOT EXISTS inspect.items (
            ms          bigint  NOT NULL,
            a           bigint  NOT NULL,
            d           bigint  NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS inspect.history (
            floatid     bigint  NOT NULL,
            a           bigint  NOT NULL,
        ...
        ALTER TABLE inspect.items ADD COLUMN IF NOT EXISTS floatid BIGINT;
        ALTER TABLE inspect.items ADD COLUMN IF NOT EXISTS price INTEGER;
        ALTER TABLE inspect.items ADD COLUMN IF NOT EXISTS listed_price INTEGER;
        ALTER TABLE inspect.items ADD COLUMN IF NOT EXISTS keychains JSONB;
        ALTER TABLE inspect.history ADD COLUMN IF NOT EXISTS price INTEGER;
        ...
        BEGIN
            ...
            IF NEW.a < OLD.a THEN
                -- If we find an older asset id than the current, still want to add it to inspect.history if not there
                INSERT INTO inspect.history VALUES (NEW.floatid, NEW.a, NEW.ms, NEW.created_at, NULL) ON CONFLICT DO NOTHING;
            ...
            IF (is_steamid(OLD.ms) AND OLD.ms != NEW.ms) OR OLD.price IS NOT NULL THEN
                -- We care about inspect.history for inventory changes or market listings that had price data
                INSERT INTO inspect.history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.created_at, OLD.price);
            END IF;
        ...
        CREATE TRIGGER extend_history_trigger
            BEFORE UPDATE ON inspect.items
            FOR EACH ROW
        ...
        CREATE TRIGGER ensure_floatid_trigger
            BEFORE INSERT ON inspect.items
            FOR EACH ROW
            EXECUTE PROCEDURE ensure_floatid();
        `);

    await this.pool
      .query(`CREATE INDEX IF NOT EXISTS i_stickers ON inspect.items USING gin (stickers jsonb_path_ops) 
                                WHERE stickers IS NOT NULL`);
    await this.pool.query(
      `CREATE INDEX IF NOT EXISTS i_paintwear ON inspect.items (paintwear)`
    );
    await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_item ON 
                                inspect.items (defindex, paintindex, paintwear, paintseed)`);
    await this.pool.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS i_unique_fid ON inspect.items (floatid)`
    );
  }
  ...
  async handleBulkInsert(data) {
    ...
    for (let [item, price] of data) {
      ...
      //   winston.debug(`Skipping item with floatvalue ${item.floatvalue}`);
      //   continue;
      // }
      ...
      const keychains =
        item.keychains.length > 0
          ? item.keychains.map((s) => {
      ...
      // Special case for charms, we store the paintwear in the paintseed field, so it is "unique"
      if (keychains.length > 0 && item.paintwear === 0) {
        item.paintwear = parseInt(item.m.slice(-9));
      }
    ...
    if (values.length === 0) {
      winston.debug("No items to insert, skipping");
      return;
    }
    ...
    try {
      ...
      winston.debug(`Inserted/updated ${values.length} inspect.items`);
    } catch (e) {
      winston.warn(e);
    }
  }
  ...
  static buildQuery(itemCount) {
    ...
    for (let c = 0; c < itemCount; c++) {
      values.push(
        `($${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}, $${i++}::jsonb, $${i++}::jsonb, now(), $${i++}, NULL, $${i++})`
      );
    }

    return `INSERT INTO inspect.items (ms, a, d, paintseed, paintwear, defindex, paintindex, stattrak, souvenir, props, stickers, keychains, created_at, rarity, floatid, price)
                VALUES ${values.join(
                  ", "
                )} ON CONFLICT(defindex, paintindex, paintwear, paintseed) DO UPDATE SET ms=excluded.ms, a=excluded.a, d=excluded.d, stickers=excluded.stickers, keychains=excluded.keychains, created_at=now()`;
  }

  updateItemPrice(assetId, price) {
    return this.pool.query(`UPDATE inspect.items SET price = $1 WHERE a = $2`, [
      price,
      assetId,
    ]);
  }
