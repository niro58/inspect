...
class Bot extends EventEmitter {
  ...
  bindEventHandlers() {
    this.steamClient.on("error", (err) => {
      ...
      // AccountLoginDeniedThrottle (eresult=87): Steam is rate-limiting this IP.
      // Retry after 5 minutes to let the throttle clear.
      if (err.eresult === 87 || (err.message && err.message.includes("AccountLoginDeniedThrottle"))) {
        const retryMs = 5 * 60 * 1000;
        winston.warn(`${this.username} throttled by Steam, retrying in 5 min`);
        setTimeout(() => this.logIn(), retryMs);
      }
    });
    ...
    this.csgoClient.on("inspectItemInfo", (itemData) => {
      if (this.resolve && this.currentRequest) {
        winston.debug(`${this.username} Received itemData`);
