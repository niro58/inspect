...
class Postgres {
  ...
  async ensureSchema() {
    await this.pool.query(`CREATE SCHEMA IF NOT EXISTS inspect;
  
          CREATE TABLE IF NOT EXISTS inspect.items (
              ms          bigint  NOT NULL,
              a           bigint  NOT NULL,
              d           bigint  NOT NULL,
          ...
          CREATE OR REPLACE FUNCTION inspect.is_steamid(IN val bigint)
              RETURNS boolean
              LANGUAGE 'plpgsql'
              IMMUTABLE
              PARALLEL SAFE
          ...
          CREATE OR REPLACE FUNCTION inspect.extend_history()
          RETURNS TRIGGER
          AS $$
          BEGIN
              ...
              IF (inspect.is_steamid(OLD.ms) AND OLD.ms != NEW.ms) OR OLD.price IS NOT NULL THEN
                  INSERT INTO inspect.history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated, OLD.price);
              END IF;
  
              IF NEW.price = OLD.price OR NEW.listed_price = OLD.listed_price OR inspect.is_steamid(NEW.ms) THEN
                  NEW.price = NULL;
          ...
          CREATE TRIGGER extend_history_trigger
              ...
              FOR EACH ROW
              EXECUTE PROCEDURE inspect.extend_history();
  
          CREATE OR REPLACE FUNCTION inspect.ensure_floatid()
          RETURNS TRIGGER
          AS $$
          BEGIN
              IF NEW.floatid IS NULL THEN
          ...
          CREATE TRIGGER ensure_floatid_trigger
              ...
              FOR EACH ROW
              EXECUTE PROCEDURE inspect.ensure_floatid();
          `);
