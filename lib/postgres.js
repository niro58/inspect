...
class Postgres {
    ...
    await this.pool.query(`CREATE TABLE IF NOT EXISTS inspect.items (
        ...
        BEGIN
            ...
            IF NEW.a < OLD.a THEN
                ...
                INSERT INTO inspect.history VALUES (NEW.floatid, NEW.a, NEW.ms, NEW.updated_at, NULL) ON CONFLICT DO NOTHING;
            ...
            IF (is_steamid(OLD.ms) AND OLD.ms != NEW.ms) OR OLD.price IS NOT NULL THEN
                ...
                INSERT INTO inspect.history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated_at, OLD.price);
            END IF;
  ...
  static buildQuery(itemCount) {
    ...
    return `INSERT INTO inspect.items (ms, a, d, paintseed, paintwear, defindex, paintindex, stattrak, souvenir, props, stickers, keychains, updated_at, rarity, floatid, price)
                VALUES ${values.join(
                  ", "
                )} ON CONFLICT(defindex, paintindex, paintwear, paintseed) DO UPDATE SET ms=excluded.ms, a=excluded.a, d=excluded.d, stickers=excluded.stickers, keychains=excluded.keychains, updated_at=now()`;
  }
